...
class Client(object):
    ...
    def connect_to_server(self):
        ...
        print(f"INFO:\t client successfully connected to server")

    def setup_communication(self):
        self.encrypt_mode = input("CBC/ECB?")
        self.socket.sendall(self.encrypt_mode.encode('UTF-8'))
    ...
    def receive_msg(self):
        encrypted_answer = self.socket.recv(1024)
        answer = self.decrypt_msg(encrypted_answer.decode("UTF-8"))
        print(f"INFO:\t First node said: '{answer}' [decrypted]")

    def send_msg(self):
        msg = input(" MSG:\t ")
        print(f"INFO:\t Sending message to second node: '{msg}' [decrypted]")
        encrypted_msg = self.encrypt_msg(msg)
        self.socket.sendall(encrypted_msg.encode("UTF-8"))
